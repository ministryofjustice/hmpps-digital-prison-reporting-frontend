...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.setLibrary(
    ...
    markdownIt({
      html: true,
      breaks: false,
      highlight: (str, language) => (language ? hljs.highlight(str, { language }).value : str),
    })
      .disable('code')
  ...
  eleventyConfig.addShortcode('example', function (exampleName, height) {
    const { data, content: nunjucksCode } = matter(
      fs.readFileSync(path.join(__dirname, 'docs', 'examples', exampleName, 'index.njk'), 'utf8').trim(),
    )
    ...
    let jsCode = ''
    try {
      jsCode = fs.readFileSync(path.join(__dirname, 'docs', 'examples', exampleName, 'script.js'), 'utf8').trim()
    } catch (e) {}
