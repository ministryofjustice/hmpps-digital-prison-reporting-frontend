...
- Dashboards visualiation definition: Removed constraint requiring dataset column id of `ts` for timeseries charts. Instead reads the `type` field on the Dashboards visualiation column definition to identify the ts column ID where the type is `timestamp`.
...
- List template: Support wrapping of long column names
...
Only specific functions are now available to be imported normally, and can all be found either at root level, e.g.:

`import { initCatalogue } from '@ministryofjustice/hmpps-digital-prison-reporting-frontend'`

or at one level down, e.g.:

`import { initCatalogue } from '@ministryofjustice/hmpps-digital-prison-reporting-frontend/catalogueUtils'`

If your app build does not tree-shake, it is recommended to use the latter format.

The full set of exports are available to view in the [rollup config file](./rollup.config.ts) - see `exports` in the `publishPkg`. Both CJS and ESM exports are provided, along with types.

There are also now two type-only exports:

`/api` - this is a re-export of the types generated by OpenAPI of the backend, in the state it was in at the time of library release.
...
You will then be able to get type inference on the injected `locals`.

**The ability to import our internal functions to get a list of reports and render them synchronously has been removed.** The (deprecated) documentation for this is available [here](https://ministryofjustice.github.io/hmpps-digital-prison-reporting-frontend/integration-guides/integrating-the-fe/#render-report-as-list) so that you can find what needs to be changed. You should instead request a report either via href or the catalogue component.
...
- Refactored report templating:
  - improved accessibility through removing dependency on nesting tables to produce reports
  - improved UI elements in report templates
  - improved Int tests for report templates
