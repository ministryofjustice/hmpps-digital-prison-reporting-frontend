context('Request missing report', () => {
  ...
  it('should show the form, and on successful submission, redirect to the success submitted page', () => {
    ...
    cy.findByRole('link', { name: /Please fill out the reason for needing the report/ }).should('be.visible').click()
    cy.findByRole('textbox', { name: /Request this report/ }).should('be.focused')
    cy.findAllByRole('paragraph').contains(/Please fill out the reason for needing the report/).should('be.visible')
  })
})
